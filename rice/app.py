...
def load_model_automatically():
    global model
    # 替换为你的模型文件实际路径（如 './rice_disease_model.pth'）
    model_path = 'best_rice_model.pth'
    if os.path.exists(model_path):
        model = torch.load(model_path, map_location=torch.device('cpu'))
        model.eval()
        print(f"模型自动加载成功: {model_path}")
    else:
        print(f"警告：未找到模型文件 {model_path}，预测功能不可用")
...
def predict():
    if model is None:
        return api_response(status="error", message="模型未加载")

    try:
        # 检查是否有文件上传
        if 'image' not in request.files:
            return api_response(status="error", message="未上传图片")

        file = request.files['image']
        if file.filename == '':
            return api_response(status="error", message="未选择图片")
        ...
        return api_response(status="error", message=f'预测失败: {str(e)}')
